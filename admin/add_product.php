...
<body>
  ...
  <label style="margin-right: 10px;" for="product:unit">Unit Type:</label>
  <!-- Display the unit name -->
  <span style="position:relative; left: 35px;" id="unit-display">
    <?php echo isset($unit_name) ? $unit_name : 'PCS'; ?>
  </span>
  <!-- Hidden input for submitting the unit_id -->
  <input type="hidden" id="unit-id" name="cmbProductUnit" value="<?php echo isset($unit_id) ? $unit_id : 2; ?>">
</div>
  ...
  <?php while ($row_selectUnit = mysqli_fetch_array($result_selectUnit)) { ?>
    <div>
      <input
        type="radio"
        id="unit_<?php echo $row_selectUnit['id']; ?>"
        name="cmbProductUnit"
        value="<?php echo $row_selectUnit['id']; ?>"
        style="margin-right: 5px;"
      >
      <label for="unit_<?php echo $row_selectUnit['id']; ?>">
        <?php echo $row_selectUnit['unit_name']; ?>
      </label>
    </div>
  <?php } ?>
</div> -->
